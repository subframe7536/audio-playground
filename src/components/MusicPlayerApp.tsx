import { createSignal, createMemo, createEffect, onCleanup, Show, ErrorBoundary } from 'solid-js'
...
export function MusicPlayerApp(props: MusicPlayerAppProps) {
  // Internal file state
  const [audioFile, setAudioFile] = createSignal<File | undefined>()

  // Debounced file processing to prevent excessive re-renders
  const [debouncedAudioFile, setDebouncedAudioFile] = createSignal<File | undefined>()

  // Debounce timeout
  let debounceTimeout: ReturnType<typeof setTimeout> | null = null

  // Debounce audio file changes
  createEffect(() => {
    if (debounceTimeout) {
    ...
    debounceTimeout = setTimeout(() => {
      setDebouncedAudioFile(audioFile())
    }, 300) // 300ms debounce
  })
  ...
  const handleDemoMode = async () => {
    try {
      // Fetch the test audio file from public directory
      const response = await fetch('/test.ogg')
      if (!response.ok) {
      ...
      const file = new File([blob], 'test.ogg', { type: 'audio/ogg' })

      setAudioFile(file)
    } catch (error) {
      console.error('Failed to load demo audio:', error)
      alert('Failed to load demo audio file. Please try uploading your own file.')
    }
  }

  return (
    <div class={`music-player-app ${props.className || ''}`}>
      <ErrorBoundary fallback={(err, reset) => <ErrorFallback error={err} reset={reset} />}>
        <PlayerProvider audioFile={debouncedAudioFile()} autoPlay={false}>
          <PlayerInterface
